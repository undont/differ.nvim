...
- Opening a diff no longer freezes the editor on a wholly rewritten file or on a minified line. The word-level pass compared every old line against every new one and re-tokenised both lines on each comparison, then gridded every token pair of each line it matched, so a 1000-line rewrite took eleven seconds and a 50KB minified line took forty-five. Both costs were paid again on every re-render, which includes a file switch, `d=`/`d-`, and a layout toggle. Lines are now tokenised once per pass, the tokens shared at the head and tail of a pair are matched without reaching the grid at all, and both grids have a ceiling past which the pass degrades to whole-line highlighting rather than grinding. The same rewrite renders in under a second and the minified line in hundredths of one, and a regenerated data fixture large enough to have exhausted memory now returns immediately
- `de` opens the file you asked for. The buffer lookup treated its path as a file-pattern rather than a literal, so a path holding a regex metacharacter resolved to whichever buffer the pattern happened to match: `a[1].lua` would open `a1.lua`, silently, and only after the session had already been torn down. The same lookup refreshes a buffer after a revert or discard, where it was checking the wrong buffer and leaving the file differ had just rewritten showing stale content. Paths now match by name, symlinks included, so the PR zoom-edit also reuses an already-open buffer on a symlinked path instead of refusing to reload it
- `:Differ sidecar stop` stops the sidecar. The binary traps `SIGTERM` only to cancel a context, and its read loop sits in a blocking read of stdin that never observes one, so the signal left the process running until nvim exited. It is now shut down by closing its stdin, which is the read loop's own exit condition. Stopping and immediately issuing another request no longer strands a process either: the one that was dying used to clear the handle belonging to its replacement, orphaning that process for the rest of the session
- The cursor keeps its column when a diff opens, and when the worktree watcher re-sources it after a `:w`. It landed on the right line but at the start of it, so opening from partway through a line lost your place. The column is carried only where the landing is exact: where the view snaps to the nearest hunk instead, or the row has no line of its own on the new side, it still starts at column 0, since the old column would point into unrelated text
