...
- The PR overview can now be commented on without leaving it: `ga` posts a comment on the pull request, and `gp` answers whatever is under the cursor, replying into a code thread or quoting a conversation comment into a new one. In visual mode `gp` quotes just the selection
