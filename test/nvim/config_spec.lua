...
describe("config.resolve context", function()
    it("keeps a valid context as written, 0 included", function()
        assert.are.equal("full", config.resolve(nil).context)
        assert.are.equal(math.huge, config.resolve({ context = math.huge }).context)
        assert.are.equal(0, config.resolve({ context = 0 }).context)
        assert.are.equal(3, config.resolve({ context = 3 }).context)
    end)

    it("puts an invalid context back to its default", function()
        assert.are.equal("full", config.resolve({ context = -1 }).context)
