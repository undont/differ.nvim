...
describe("pr.threads.collapsed_state", function()
    it("keeps an expanded thread expanded either side of the cursor", function()
        assert.is_false(threads.collapsed_state(nil, "expanded", false))
        assert.is_false(threads.collapsed_state(nil, "expanded", true))
    end)

    it("opens a peek thread on its anchor row alone", function()
        assert.is_true(threads.collapsed_state(nil, "peek", false))
        assert.is_false(threads.collapsed_state(nil, "peek", true))
    end)

    it("rests markers collapsed, which the float's group_active=true then opens", function()
        assert.is_true(threads.collapsed_state(nil, "markers", false))
        assert.is_false(threads.collapsed_state(nil, "markers", true))
    end)

    it("lets the gc override win over both the display mode and the peek", function()
        assert.is_true(threads.collapsed_state(true, "expanded", true))
