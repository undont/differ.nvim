...
describe("pr.threads.collapsed_state", function()
    it("collapses a resting-collapsed thread off its anchor row, expands it on", function()
        assert.is_true(threads.collapsed_state(nil, true, false))
        assert.is_false(threads.collapsed_state(nil, true, true))
    end)

    it("keeps a resting-expanded thread expanded either side of the cursor", function()
        assert.is_false(threads.collapsed_state(nil, false, false))
        assert.is_false(threads.collapsed_state(nil, false, true))
    end)

    it("lets the gc override win over both the resting state and the peek", function()
        assert.is_true(threads.collapsed_state(true, false, true))
        assert.is_false(threads.collapsed_state(false, true, false))
    end)
end)
